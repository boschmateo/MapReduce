...
if __name__ == "__main__":
    ...
    host = create_host('http://10.110.175.107:1679')

    #Spawn the reducer
    reducerHost = host.lookup_url('http://10.110.178.22:' + str(MAX_PORT_VALUE) + '/', Host)
    ...
    for port in range(MIN_PORT_VALUE, MAX_PORT_VALUE):
        ...
        remoteHost = host.lookup_url('http://10.110.178.22:' + str(port) + '/', Host)
        #Add the slaves into the list
        remoteHostList.append(remoteHost.spawn(port, 'map/Map'))

    #Testing the values
    for pos in range(numberOfSpawns):
        remoteHostList[pos].map(mode, "http://10.110.175.107:8000/" + str(pos) + ".part", reducer)
