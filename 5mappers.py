...
if __name__ == "__main__":
    ...
    host3 = remoteHost.spawn(1280, 'map/Map')

    remoteHost = host.lookup_url(IP_COMPUTER2 +':' + str(1281) + '/', Host)
    host4 = remoteHost.spawn(1281, 'map/Map')

    remoteHost = host.lookup_url(IP_COMPUTER2 +':' + str(1282) + '/', Host)
    host5 = remoteHost.spawn(1282, 'map/Map')
    ...
    host5.map(mode, IP_COMPUTER1+':8000/' + str(4) + ".part", reducer)
